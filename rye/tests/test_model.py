...
def test_batched_forward():
    ...
    big_batch_size = 50
    batch_size = 10
    invariant_input = torch.randn(big_batch_size, batch_size, input_size)
    equivariant_input = torch.randn(big_batch_size, batch_size, 3)
    invariant_hidden = torch.randn(big_batch_size, batch_size, hidden_size)
    equivariant_hidden = torch.randn(big_batch_size, batch_size, 3, num_channels)
    probability = torch.rand(big_batch_size, batch_size, batch_size).softmax(dim=-1)
    ...
    assert invariant_hidden.shape == (big_batch_size, repeat, length, batch_size, hidden_size)
    assert equivariant_hidden.shape == (big_batch_size, repeat, length, batch_size, 3, num_channels)
